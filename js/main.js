...
const apiKey = 'apikey=7035c60c&s'
...
async function getMovie(title) {
    try {const { data } = await axios ({
        url: `https://www.omdbapi.com?${apiKey}=${title}&type=movie`,
        method: 'GET'
        })
        ...
        //박스에 내용채워넣기
        const searchPoster = document.querySelector(`.searchPage .searchBox${i} .poster img`)
        const searchTitle = document.querySelector(`.searchPage .searchBox${i} .title div`)
        const searchYear = document.querySelector(`.searchPage .searchBox${i} .year span`)
...
async function getMoviePage(title, page, pageID) {
    try {
        const { data } = await axios ({
            url: `https://www.omdbapi.com?${apiKey}=${title}&type=movie&page=${page}`,
            method: 'GET'
        })
        ...
        for (let i = 0; i < movies.length; i++) {
            ...
            const createNewSearchBox = document.createElement('div')
            createNewSearchBox.classList.add('searchBox',`searchBox${i}pageID${pageID}`)
            createNewSearchBox.innerHTML = `<div class="poster"><img src="" alt=""></div>
                <div class="title"><div></div></div>
                <div class="year"><span></span></div>`
            const searchPage = document.querySelector('.searchPage')
            ...
            //박스에 내용채워넣기
            const searchPoster = document.querySelector(`.searchPage .searchBox${i}pageID${pageID} .poster img`)
            const searchTitle = document.querySelector(`.searchPage .searchBox${i}pageID${pageID} .title div`)
            const searchYear = document.querySelector(`.searchPage .searchBox${i}pageID${pageID} .year span`)
